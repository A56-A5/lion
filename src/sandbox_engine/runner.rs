...
pub fn run_sandboxed(
    ...
    let _ = ctrlc::set_handler(move || {
        ...
        let _ = Command::new("kill").arg("-TERM").arg(bwrap_pid.to_string()).status();
    });
    ...
    if use_tui {
        ...
        for _ in 0..20 {
            if let Some(p) = get_direct_child(bwrap_pid) { leader_pid = Some(p); break; }
        ...
        let leader_pid = leader_pid.unwrap_or(bwrap_pid);
        let _perf = crate::monitor::perf::PerfHandle::spawn(leader_pid, &cmd.first().cloned().unwrap_or_default());
    ...
    if status.success() {
        ...
        if code == 1 || code == 126 || code == 127 {
            if let Some(path) = find_binary(&program_name) {
                if !is_executable(&path) { return Err(LionError::PermissionDenied(program_name)); }
            } else {
...
fn find_binary(name: &str) -> Option<PathBuf> {
    ...
    if let Ok(path_var) = env::var("PATH") {
        for dir in path_var.split(':') {
            ...
            if full_path.exists() { return Some(full_path); }
        }
    }
    None
}
