...
impl App {
    ...
    pub fn on_key(&mut self, key: crossterm::event::KeyCode) {
        ...
        match key {
            K::Char('q') | K::Char('Q') => self.should_quit = true,
            K::Tab                       => self.current_tab = self.current_tab.next(),
            K::BackTab                   => self.current_tab = self.current_tab.prev(),
            K::Char('1')                 => self.current_tab = Tab::Events,
            K::Char('2')                 => self.current_tab = Tab::Perf,
            K::Char('3')                 => self.current_tab = Tab::Status,
            K::Down | K::Char('j')       => {
